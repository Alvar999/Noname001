...
public OnPlayerUpdate(playerid)
{
	...
	for(new Sz; Sz < MAX_SZ; Sz++)
 	{
  		if(IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
  		{
			...
			if(EnterSafeZones[playerid] < 0)
            {
                ...
                SendClientMessageEx(playerid, COLOR_WHITE, "[{FF000B}SAFEZONE{FFFFFF}] Ban da vao ben trong khu vuc an toan.");
				TextDrawShowForPlayer(playerid, SafeZone_[0]);
	            TextDrawShowForPlayer(playerid, SafeZone_[1]);
	            TextDrawShowForPlayer(playerid, SafeZone_[2]);
	            TextDrawShowForPlayer(playerid, SafeZone_[3]);
            }
  		...
  		if(Sz == EnterSafeZones[playerid] && EnterSafeZones[playerid] >= 0)
        {
            if(!IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
            {
                ...
                SendClientMessageEx(playerid, COLOR_WHITE, "[{FF000B}SAFEZONE{FFFFFF}] Ban da ra khoi khuc vuc an toan.");
                TextDrawHideForPlayer(playerid, SafeZone_[0]);
                TextDrawHideForPlayer(playerid, SafeZone_[1]);
                TextDrawHideForPlayer(playerid, SafeZone_[2]);
                TextDrawHideForPlayer(playerid, SafeZone_[3]);				
            }
            break;
        }
 	}
    ...
    new stringtd[200];
	format(stringtd, 200, "Da Quy: %d", PlayerInfo[playerid][pCredits]);
	PlayerTextDrawSetString(playerid,Daquy[playerid][1], stringtd);
    if(GiaoCa[playerid] == 1)
		{
			SetPlayerAttachedObject(playerid, 1, 1607, 1, 0.097999, 0.361999, 0, 93, 0, 0, 0.430999, 0.36, 0.382);
		}
	...
	/*if(glob2B74[playerid] == 3)
	{
		new var0[200];
		format(var0, 200, "[Hoa Binh]");
		SetPlayerChatBubble(playerid, var0, 869072810, 1112014848, 10000);
	}
	if(glob2B74[playerid] == 2)
	{
		new var0[200];
		format(var0, 200, "[TDM]");
		SetPlayerChatBubble(playerid, var0, 869072810, 1112014848, 10000);
	}
	if(glob2B74[playerid] == 1)
	{
		new var0[200];
		format(var0, 200, "[Roleplay]");
		SetPlayerChatBubble(playerid, var0, 869072810, 1112014848, 10000);
	}*/
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
    ...
    if(Text:INVALID_TEXT_DRAW == clickedid) {
	    if(GetPVarInt(playerid, "MenuSAMP") == 1) {
	        ExitMenuSAMP(playerid);
		}
    }
...
public OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
	if(GetPVarInt(playerid, "mS_list_active") == 1 || (GetTickCount()-GetPVarInt(playerid, "mS_list_time")) > 200)
	{
		...
		if(playertextid == MenuSAMP[playerid][1])
    {
				//SHOW THONG TIN
                if(gPlayerLogged{playerid} != 0) ShowStats(playerid, playerid);
                //HIDE
                PlayerTextDrawHide(playerid, MenuSAMP[playerid][0]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][1]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][2]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][3]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][4]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][5]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][6]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][7]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][8]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][9]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][10]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][11]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][12]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][13]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][14]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][15]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][16]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][17]);
				PlayerPlaySound(playerid, 1136, 0,0,0);
				CancelSelectTextDraw(playerid);
    }
    if(playertextid == MenuSAMP[playerid][2])
    {
				//SHOW Tui Do
                 new str[500], name[1280];
			     new vatlieu = PlayerInfo[playerid][pMats];
			     new money = PlayerInfo[playerid][pCash];
			     new pot = PlayerInfo[playerid][pPot];
			     new crack = PlayerInfo[playerid][pCrack];
			     new credits = PlayerInfo[playerid][pCredits];
			     new coin = PlayerInfo[playerid][pDaQuy];
			     format(str, sizeof(str), "\n{00FF00}Coin : %d\nMoney : %d\nVat Lieu : %d\nCredits : %d\nPot : %d\nCrack : %d", coin, money, vatlieu, credits, pot, crack);
    ...
    if(playertextid == MenuSAMP[playerid][3])
    {
				//SHOW Chinh Xe
				...
				if(PlayerInfo[playerid][pFreezeCar] == 0 || PlayerInfo[playerid][pAdmin] >= 2)
				{
					...
					ShowPlayerDialog(playerid, VEHICLESTORAGE, DIALOG_STYLE_LIST, "Xe cua toi", vstring, "Dong Y", "Huy bo");
				}
    ...
    if(playertextid == MenuSAMP[playerid][4])
    {
				//SHOW Bao Cao
			    if(GetPVarType(playerid, "HasReport")) {
					SendClientMessageEx(playerid, COLOR_GREY, "Ban chi co the co 1 bao cao. (/huybaocao)");
				}
			 	else if(JustReported[playerid] > 0)
				{
					SendClientMessageEx(playerid, COLOR_GREY, "Ban phai cho 25s de co the tiep tuc bao cao!");
					return 1;
				}
			 	else if(PlayerInfo[playerid][pAdmin] >= 2 && PlayerInfo[playerid][pAdmin] < 1337) {
    ...
    if(playertextid == MenuSAMP[playerid][5])
    {
				//SHOW TRANG BI
    ...
    if(playertextid == MenuSAMP[playerid][6])
    {
				//SHOW TRANG BI
    ...
    if(playertextid == MenuSAMP[playerid][7])
    {
				//SHOW TRANG BI
                ShowPlayerDialog(playerid, TIMVIECLAM, DIALOG_STYLE_LIST, "Ban can tim viec lam ?","Tham Tu\nLuat Su\nGai diem\nBan thuoc phien\nVe Si\nTho sua xe\nBan vu khi\nVo Si\nTai Xe Taxi\nBuon thuoc phien\nTho Thu Cong\nNguoi pha ruou\nVan Chuyen Banh\nDon Rac\nTai Xe Trucker", "Chon", "Huy bo");
                //HIDE
                PlayerTextDrawHide(playerid, MenuSAMP[playerid][0]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][1]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][2]);
    ...
    if(playertextid == MenuSAMP[playerid][8])
    {
				//SHOW TIMDUONG
				ShowPlayerDialog(playerid, TIMDUONG, DIALOG_STYLE_LIST, "Ban Muon Tim Duong","Vip San Fire\nSieu Thi 24/7\nNgan Hang San Fire\nDmv (noi dong tien phat)\nHQ Taxi\nShop Quan Ao\nBenh Vien San Fire\nShop Vu Khi\nCua Hang Xe CU\nHai Can\nChe Bien Can Sa\nBan Can Sa\nLam sach Rau\nBan Rau\nTru So Canh Sat San Fire\nCity Hall\nLay Vat Lieu/nThi bang Lai\nTham Gia Arena", "Dong Y", "Bo Qua");
                //HIDE
                PlayerTextDrawHide(playerid, MenuSAMP[playerid][0]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][1]);
				PlayerTextDrawHide(playerid, MenuSAMP[playerid][2]);
...
public OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid)
{
	if(InfoMessage[playerid] == true) // version 2
	{
       if(issuerid != INVALID_PLAYER_ID)
       {
        ...
        format(infoString, sizeof(infoString), "{FF0005}-%.02f{FFFFFF} |{0015B4} %s{FFFFFF} | {FFAD00}%s{FFFFFF} | Vi tri tan cong: %s.", amount, weaponName, attackerName, GetBodyPartName(bodypart));
...
public OnPlayerGiveDamage(playerid, damagedid, Float:amount, weaponid)
{
	...
	/*if(glob2B74[playerid] == 3)
	{
		GameTextForPlayer(playerid, "~r~Ban Dang O Che Do Hoa Binh Khong The Tan Cong Nguoi Khac", 5000, 3);
		GetPlayerHealth(playerid, var1);
		SetPlayerHealth(playerid, var1 - 50);
	}
	if(IsPlayerConnected(var0))
	{
		if(!glob2B74[playerid] == 3)
		{
		    GameTextForPlayer(playerid, "~r~Nguoi Do Dang O Che Do Hoa Binh", 5000, 3);
			GetPlayerHealth(playerid, var1);
			SetPlayerHealth(playerid, var1 - 50);
		}
	}*/
    ...
    GetWeaponName(weaponid, weaponname, sizeof (weaponname));
    format(string, sizeof(string), " {07FF00}+%.02f{FFFFFF} | {FFE400}%s{FFFFFF} | {3B6B00} %s {FFFFFF}| Vi tri tan cong: %s.", amount, weaponname, victim, GetBodyPartName(bodypart));
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	foreach(new i: Player)
	{
	         if(playerid == i)
		     {
						format(string, sizeof(string), "{FFB200}[Trong Xe] %s {FFFFFF}noi: %s", sendername, text);
	...
	if(!ispassenger)
	{
		...
		else if(XeGiaoXang(vehicleid))
		{
		    if(PlayerInfo[playerid][pJob] != 29 && PlayerInfo[playerid][pJob2] != 29)
		    {
			    new Float:slx, Float:sly, Float:slz;
				GetPlayerPos(playerid, slx, sly, slz);
				SetPlayerPos(playerid, slx, sly, slz+1.3);
			    ...
			    RemovePlayerFromVehicle(playerid);
			    defer NOPCheck(playerid);
			    SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong phai nguoi van chuyen xang dau!");
			}
		}
...
{
     if(IsPlayerInAnyVehicle(playerid) && GetPlayerVehicleID(playerid) == GetPVarInt(playerid, "hoclaixe_vehicleid"))
	{
	    if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 0)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 1);
			SetPlayerRaceCheckpoint(playerid, 0, 1356.9756,-1418.1904,13.3906, 5);
			GameTextForPlayer(playerid, "STEP 1", 2500, 4);
		}
	    else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 1)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 2);
			SetPlayerRaceCheckpoint(playerid, 0, 1356.9756,-1418.1914,13.3906, 5);
			GameTextForPlayer(playerid, "STEP 2", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 2)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 3);
			SetPlayerRaceCheckpoint(playerid, 0, 815.6445,-1396.0615,13.4323, 5);
			GameTextForPlayer(playerid, "STEP 3", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 3)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 4);
			SetPlayerRaceCheckpoint(playerid, 0, 324.6555,-1627.9172,33.1630, 5);
			GameTextForPlayer(playerid, "STEP 4", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 4)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 5);
			SetPlayerRaceCheckpoint(playerid, 0, 1037.9220,-1830.6761,13.5667, 5);
			GameTextForPlayer(playerid, "STEP 5", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 5)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 6);
			SetPlayerRaceCheckpoint(playerid, 0, 1033.1072,-2260.3286,12.9516, 5);
			GameTextForPlayer(playerid, "STEP 6", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 6)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 7);
			SetPlayerRaceCheckpoint(playerid, 0, 1346.4745,-2466.9119,7.6563, 5);
			GameTextForPlayer(playerid, "STEP 7", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 7)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 8);
			SetPlayerRaceCheckpoint(playerid, 0, 1329.5033,-2524.1174,13.3750, 5);
			GameTextForPlayer(playerid, "STEP 8", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 8)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 9);
			SetPlayerRaceCheckpoint(playerid, 0, 2172.8113,-2505.0293,13.3750, 5);
			GameTextForPlayer(playerid, "STEP 9", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 9)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 10);
			SetPlayerRaceCheckpoint(playerid, 0, 2816.1235,-2045.9063,10.9375, 5);
			GameTextForPlayer(playerid, "STEP 10", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 10)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 11);
			SetPlayerRaceCheckpoint(playerid, 0, 2281.2524,-2073.6370,13.3831, 5);
			GameTextForPlayer(playerid, "STEP 11", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 11)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 12);
			SetPlayerRaceCheckpoint(playerid, 0, 2193.3940,-1734.2450,13.3875, 5);
			GameTextForPlayer(playerid, "STEP 12", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 12)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 13);
			SetPlayerRaceCheckpoint(playerid, 0, 833.2135,-1749.4635,13.3845, 5);
			GameTextForPlayer(playerid, "STEP 13", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 13)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 14);
			SetPlayerRaceCheckpoint(playerid, 0, 1558.4591,-1735.3134,13.3845, 5);
			GameTextForPlayer(playerid, "STEP 14", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 14)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 15);
			SetPlayerRaceCheckpoint(playerid, 0, 1484.0479,-1743.4606,13.5469, 5);
			GameTextForPlayer(playerid, "STEP 15", 2500, 4);
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 15)
		{
			DisablePlayerRaceCheckpoint(playerid);
			SetPVarInt(playerid, "hoclaixe_thuchanh", 16);
			SetPlayerRaceCheckpoint(playerid, 0, 1484.0479,-1743.4626,13.5469, 5);
			GameTextForPlayer(playerid, "STEP 16", 4000, 4);
			SendClientMessageEx(playerid, -1, "{FF0033}RL-RP System{FFFFFF}: Hay de xe ve vi tri ban dau");
		}
		else if(GetPVarInt(playerid, "hoclaixe_thuchanh") == 16)//finish
		{
		    new Float: vheal;
		    GetVehicleHealth(GetPVarInt(playerid, "hoclaixe_vehicleid"), vheal);
		    if(vheal < 1000)
		    {
				if(900 < vheal <= 999)
				{
		            SetPVarInt(playerid, "hoclaixe_diemthuchanh", 9);
				}
				else if( 800< vheal <= 899)
				{
				    SetPVarInt(playerid, "hoclaixe_diemthuchanh", 8);
				}
				else if( 700 < vheal <= 799)
				{
				    SetPVarInt(playerid, "hoclaixe_diemthuchanh", 7);
				}
				else if( 600 < vheal <= 699)
				{
				    SetPVarInt(playerid, "hoclaixe_diemthuchanh", 6);
				}
				else if( 500 < vheal <= 599)
				{
				    SetPVarInt(playerid, "hoclaixe_diemthuchanh", 5);
				}
				else if(vheal <= 499)
				{
				    SetPVarInt(playerid, "hoclaixe_diemthuchanh", 1);
				}
			}
			else
			{
			    SetPVarInt(playerid, "hoclaixe_diemthuchanh", 10);
			}
			DisablePlayerRaceCheckpoint(playerid);
			DeletePVar(playerid, "hoclaixe_thuchanh");
			GameTextForPlayer(playerid, "FINISH!!!", 4000, 4);
			RemovePlayerFromVehicle(playerid);
			DestroyVehicle(GetPVarInt(playerid, "hoclaixe_vehicleid"));
			SetPlayerPos(playerid, GetPVarFloat(playerid, "hoclaixe_px"), GetPVarFloat(playerid, "hoclaixe_py"), GetPVarFloat(playerid, "hoclaixe_pz"));
			SetPlayerFacingAngle(playerid, GetPVarFloat(playerid, "hoclaixe_pa"));
			SetPlayerInterior(playerid, 3);
			PlayerInfo[playerid][pCarLic] = 1;
			new str[1024], diem, xeploai[20];
			diem = (GetPVarInt(playerid, "hoclaixe_diemthuchanh") + GetPVarInt(playerid, "traloidung_hoclaixe"))/2;
			if(diem == 10)
			{
			    xeploai = "Xuat xac";
			}
			else if(8.5 < diem <= 9.9)
			{
			    xeploai = "Kha";
			}
			else if(7.5 < diem <= 8.4)
			{
			    xeploai = "Trung binh";
			}
			else if(diem < 7.5)
			...
			format(str, 768, "{FF0033}RL-RP System{FFFFFF}: {33CCFF}Chuc mung, ban vua hoan thanh khoa thi bang lai xe");
			SendClientMessageEx(playerid, -1, str);
			format(str, 768, "Thanh tich - %s\nBan vua hoan thanh xong khoa thi bang lai xe\nVoi so diem: %d - Xep loai: %s", GetPlayerNameEx(playerid), diem, xeploai);
			ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_MSGBOX, "Thong Bao", str, "OK", "");
			DeletePVar(playerid, "hoclaixe");
			DeletePVar(playerid, "traloidung_hoclaixe");
			DeletePVar(playerid, "hoclaixe_px");
			DeletePVar(playerid, "hoclaixe_py");
			DeletePVar(playerid, "hoclaixe_pz");
			DeletePVar(playerid, "hoclaixe_pa");
			DeletePVar(playerid, "hoclaixe_vehicleid");
			DeletePVar(playerid, "hoclaixe_thuchanh");
		}
	}
    return 1;
...
public OnPlayerConnect(playerid) {
    ...
    // Nguoi choi dang nhap
	new szLog[128];
	format(szLog, sizeof(szLog), "{FF0000}[LOGIN] {00FF00}[User: %s(%i)] {00FFFF}[IP: %s] {FFFFFF}[Credits: %s]", GetPlayerNameEx(playerid), PlayerInfo[playerid][pId], GetPlayerIpEx(playerid), number_format(PlayerInfo[playerid][pCredits]));
	ABroadCast(COLOR_GREEN , szLog, 1);

	// Nhac login
	PlayAudioStreamForPlayerEx(playerid, "https://kow-vn.net/nhac/vipp.mp3");
	...
	SendClientMessage( playerid, COLOR_WHITE, "{00FF02}SB-RP.TK : {FFFFFF}Chao Mung Ban Den Voi May Chu {12FF00}[ SB-RP.TK ]." );
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    if(IsValidVehicleID(GetPVarInt(playerid, "hoclaixe_vehicleid")))
	{
	    DestroyVehicle(GetPVarInt(playerid, "hoclaixe_vehicleid"));
	}
    ...
    Status[playerid] = CreatePlayerTextDraw(playerid, 4.000000, 215.000000, "_");
	PlayerTextDrawFont(playerid, Status[playerid], 2);
	PlayerTextDrawLetterSize(playerid, Status[playerid], 0.230000, 2.299999);
	PlayerTextDrawTextSize(playerid, Status[playerid], 885.000000, -88.000000);
	PlayerTextDrawSetOutline(playerid, Status[playerid], 1);
	PlayerTextDrawSetShadow(playerid, Status[playerid], 0);
	PlayerTextDrawAlignment(playerid, Status[playerid], 1);
	PlayerTextDrawColor(playerid, Status[playerid], -1);
	PlayerTextDrawBackgroundColor(playerid, Status[playerid], 255);
	PlayerTextDrawBoxColor(playerid, Status[playerid], 50);
	PlayerTextDrawUseBox(playerid, Status[playerid], 0);
	PlayerTextDrawSetProportional(playerid, Status[playerid], 1);
	PlayerTextDrawSetSelectable(playerid, Status[playerid], 0);
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(IsPlayerConnected(playerid) && IsPlayerConnected(killerid))
	{
		...
		*/
		...
		{
		SendClientMessage(playerid, COLOR_YELLOW, "Ban da chet, he thong tu dong huy ghep");
     ...
     //   new duelmoney = Duel_Money[playerid];
	//	new DuelTaxed;
	//	new WinMoney;
 		if(Duel_Round[playerid] < 3 && Duel_Win[playerid] !=2 || Duel_Win[targetid] !=2)
  		{
  		    new string[128],string1[128];
		//	if(Duel_Round[playerid]==1) Duel_Round[playerid] +=1;
        	Duel_Round[targetid] +=1;
        	Duel_Win[targetid] +=1;
        ...
        //	ClearAnimations(playerid);
        	...
        	Duel_Round[playerid] +=1;
        	
        	format(string, sizeof(string), "[Rank]{FFFFFF} Ban da thua trong vong %d - Ti so hien tai la %d [%s] - %d [%s].", Duel_Round[playerid], Duel_Win[playerid], GetPlayerNameEx(playerid), Duel_Win[targetid], GetPlayerNameEx(targetid));
        	SendClientMessageEx(playerid,  COLOR_LIGHTBLUE, string);
            format(string, sizeof(string), "[Rank]{FFFFFF} Ban da thang trong vong %d - Ti so hien tai la %d [%s] - %d [%s].", Duel_Round[playerid], Duel_Win[playerid], GetPlayerNameEx(playerid), Duel_Win[targetid], GetPlayerNameEx(targetid));
        	SendClientMessageEx(targetid,  COLOR_LIGHTBLUE, string);

			if(Duel_Round[playerid] < 3 && Duel_Win[playerid] !=2 || Duel_Win[targetid] !=2)
			{
			    SetTimerEx("DuelStartAgain", 5000, false, "ii", playerid, targetid);
			}
			else if(Duel_Round[playerid] == 3 || Duel_Win[playerid] == 2 || Duel_Win[targetid] == 2)
			{
				if (Duel_Win[targetid] > Duel_Win[playerid])
				{
//					WinMoney = duelmoney-floatround(DuelTaxed,floatround_round);
//					PlayerInfo[playerid][pKimCuong] -= duelmoney;
  //      			PlayerInfo[targetid][pKimCuong] += WinMoney;
        			PlayerInfo[playerid][pPoints] -= 15;
        			PlayerInfo[targetid][pPoints] += 10;

        			if(PlayerInfo[playerid][pPoints] >= 400)
        			{
            			format(string1, sizeof(string1), "[Vinh Danh]{FFFFFF} %s Da Chien Thang Trong Tran Dau Rank Cao voi %s.", GetPlayerNameEx(targetid), GetPlayerNameEx(playerid));
            			SendClientMessageToAllEx(COLOR_YELLOW, string1);
        			}
        			if (PlayerInfo[targetid][pUyTin] < 100)
        			{
            			PlayerInfo[targetid][pUyTin] += 1;
            			SendClientMessageEx(targetid,  COLOR_LIGHTBLUE, "[Rank]{FFFFFF} Ban nhan duoc 1 diem uy tin");
        			}

        			format(string, sizeof(string), "[Rank]{FFFFFF}Ban da nhan duoc 10 Points sau khi chien thang tran Rank voi %s.", GetPlayerNameEx(targetid));
        			SendClientMessageEx(targetid, COLOR_LIGHTBLUE, string);

        			format(string, sizeof(string), "[Rank]{FFFFFF}Ban da mat 15 Points sau khi thua trong tran Rank voi %s.",  GetPlayerNameEx(playerid));
        			SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);
        		}
				else if(Duel_Win[targetid] < Duel_Win[playerid])
        		{
        			PlayerInfo[targetid][pPoints] -= 15;
        			...
        			//new string[128];
        			if (PlayerInfo[playerid][pUyTin] < 100)
        			{
            			PlayerInfo[playerid][pUyTin] += 1;
            			SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "[Rank]{FFFFFF}Ban nhan duoc 1 diem uy tin");
        			}
				...
				SendClientMessageEx(targetid, COLOR_WHITE, "Debug");
				SendClientMessageEx(playerid, COLOR_WHITE, "Debug");
        		...
        		SetPlayerPos(playerid, GetPVarFloat(playerid, "Duel_PosX"), GetPVarFloat(targetid, "Duel_PosY"), GetPVarFloat(targetid, "Duel_PosZ"));
        		SetPlayerFacingAngle(playerid, GetPVarFloat(targetid, "Duel_PosA"));

        		if(GetPVarInt(playerid, "Duel_VW") > 0)
        		{
            		Player_StreamPrep(playerid, GetPVarFloat(targetid, "Duel_PosX"), GetPVarFloat(targetid, "Duel_PosY"), GetPVarFloat(targetid, "Duel_PosZ"), FREEZE_TIME);
        		}
        		SetPlayerVirtualWorld(playerid, GetPVarInt(targetid, "Duel_VW"));
        		SetPlayerInterior(playerid, GetPVarInt(targetid, "Duel_Int"));
...
public OnPlayerSpawn(playerid)
{
	...
	PlayerTextDrawShow(playerid, Daquy[playerid][0]);
	PlayerTextDrawShow(playerid, Daquy[playerid][1]);
...
public OnPlayerLeaveCheckpoint(playerid)
{
	...
	if(PlayerInfo[playerid][pDangthi] == 1)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 1356.9756,-1418.1904,13.3906, 4.0);
        PlayerInfo[playerid][pDangthi] = 2;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}

    if(PlayerInfo[playerid][pDangthi] == 2)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 815.6445,-1396.0615,13.4323, 4.0);
        PlayerInfo[playerid][pDangthi] = 3;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}

    if(PlayerInfo[playerid][pDangthi] == 3)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 324.6555,-1627.9172,33.1630, 4.0);
        PlayerInfo[playerid][pDangthi] = 4;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}
	
    if(PlayerInfo[playerid][pDangthi] == 4)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 1037.9220,-1830.6761,13.5667, 4.0);
        PlayerInfo[playerid][pDangthi] = 5;
		SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}
	
    if(PlayerInfo[playerid][pDangthi] == 5)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 1033.1072,-2260.3286,12.9516, 4.0);
        PlayerInfo[playerid][pDangthi] = 6;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}
	
    if(PlayerInfo[playerid][pDangthi] == 6)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 1346.4745,-2466.9119,7.6563, 4.0);
        PlayerInfo[playerid][pDangthi] = 7;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}

    if(PlayerInfo[playerid][pDangthi] == 7)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 1329.5033,-2524.1174,13.3750, 4.0);
        PlayerInfo[playerid][pDangthi] = 8;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}

    if(PlayerInfo[playerid][pDangthi] == 8)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 2172.8113,-2505.0293,13.3750, 4.0);
        PlayerInfo[playerid][pDangthi] = 9;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}

    if(PlayerInfo[playerid][pDangthi] == 9)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 2816.1235,-2045.9063,10.9375, 4.0);
        PlayerInfo[playerid][pDangthi] = 10;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}
	
    if(PlayerInfo[playerid][pDangthi] == 10)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 2281.2524,-2073.6370,13.3831, 4.0);
        PlayerInfo[playerid][pDangthi] = 11;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}
	
    if(PlayerInfo[playerid][pDangthi] == 11)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 2193.3940,-1734.2450,13.3875, 4.0);
        PlayerInfo[playerid][pDangthi] = 12;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}

    if(PlayerInfo[playerid][pDangthi] == 12)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 1833.2135,-1749.4635,13.3845, 4.0);
        PlayerInfo[playerid][pDangthi] = 13;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fba80c}Hay di den dien mau do tiep theo nao!");
        return 1;
	}
	
    if(PlayerInfo[playerid][pDangthi] == 13)
    {
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        SetPlayerCheckpoint(playerid, 1558.4591,-1735.3134,13.3845, 4.0);
        PlayerInfo[playerid][pDangthi] = 14;
        SendClientMessageEx(playerid, COLOR_WHITE, "{fdff00}Tuyet Voi, hay ve City Hall de nhan bang lai nao!");
        return 1;
	}	

    if(PlayerInfo[playerid][pDangthi] == 14)
    {
        new vehicleid = GetPlayerVehicleID(playerid);
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
        PlayerInfo[playerid][pDangthi] = 0;
        DestroyVehicle(vehicleid);
	    GivePlayerCash(playerid,-100000);
	    PlayerInfo[playerid][pCarLic] = 1;
	    SetPlayerPos(playerid, 1484.0479,-1743.4606,13.5469);
	    PlayerInfo[playerid][pVW] = 0;
	    SetPlayerVirtualWorld(playerid, 0);
        SendClientMessageEx(playerid, COLOR_WHITE, "{0cfbb8}Ban da nhan duoc bang lai xe hoi, va bi tru 100,000$ cho tien chi phi.");
        SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "{0cfbb8}Ban da nhan duoc bang lai xe.");
        return 1;
	}
...
public OnPlayerEnterCheckpoint(playerid)
{
    ...
    if(PlayerInfo[playerid][pJob] == 36 || PlayerInfo[playerid][pJob2] == 36)
	{
		if(IsPlayerInRangeOfPoint(playerid, 3, -727.09,1524.86,39.64))
		{
            RemovePlayerAttachedObject(playerid, 9);
            DisablePlayerCheckpoint(playerid);
            DeletePVar(playerid, "DangKhaiThac");
    ...
    if(PlayerInfo[playerid][pJob] == 35 || PlayerInfo[playerid][pJob2] == 35)
	{
		if(IsPlayerInRangeOfPoint(playerid, 3, -473.1526,-103.6694,61.9928))
		{
            RemovePlayerAttachedObject(playerid, 9);
            DisablePlayerCheckpoint(playerid);
            DeletePVar(playerid, "DangDaoVang");
            SetPlayerSpecialAction(playerid, 0);
            return 1;
        }
    }
	...
	else
	{
		switch (gPlayerCheckpointStatus[playerid])
		{
			...
			{
			    if(IsPlayerInRangeOfPoint(playerid, 6, 2528.78,-2089.06,13.54) || IsPlayerInRangeOfPoint(playerid, 6, 2098.6543,-104.3568,-0.4820))
			    {
				    new vehicleid = GetPlayerVehicleID(playerid);
	...
	if(GetPVarInt(playerid, "DaNhanMonPhucVu") == 1 && IsPlayerInRangeOfPoint(playerid, 2, 1817.0023, -1562.8186, 13.5487))
	{
...
if(newkeys & KEY_YES)
{
	if(IsPlayerInRangeOfActor(playerid, Actor[8]))
		{
  			ShowPlayerDialog(playerid, BANKHOANGSAN, DIALOG_STYLE_LIST, "Menu Khoang San","Ban Fe(10$)\nBan Cu(20$)\nBan Au(500$)\n9mm(14FE)\nShotGun(32FE)\nUzi(30Cu)\nMp5(37Cu)\nAk(30Au)\nM4(30Au)", "Chon", "Thoat");
              return 1;
		}
}
...
if(newkeys & KEY_YES) 
{
	new Float:PosXACtor, Float:PosYACtor, Float:PosZACtor;
        GetActorPos(NPCJobPhucVu, PosXACtor, PosYACtor, PosZACtor);
        if(IsPlayerInRangeOfPoint(playerid, 2.0, PosXACtor, PosYACtor, PosZACtor))
        {
            if(PlayerInfo[playerid][pJob] != 37 && PlayerInfo[playerid][pJob2] != 37)
            {
       		 ShowPlayerDialog(playerid, Job_PhucVu, DIALOG_STYLE_LIST, "Cong Viec Nguoi Phuc Vu", "Xin Viec Chinh\nXin Viec Phu\nTu bo cong viec 1\nTu bo cong viec 2", "Chon", "Huy");
				return 1;
			}
			else
			{
...
{
	new Float:PosXACtor, Float:PosYACtor, Float:PosZACtor;
        GetActorPos(KhoangSanActor, PosXACtor, PosYACtor, PosZACtor);
        if(IsPlayerInRangeOfPoint(playerid, 2.0, PosXACtor, PosYACtor, PosZACtor))
        {
            if(PlayerInfo[playerid][pJob] != 36 && PlayerInfo[playerid][pJob2] != 36)
            {
       			ShowPlayerDialog(playerid, Job_KhaiThac, DIALOG_STYLE_LIST, "Cong Viec Khai Thac", "Xin Viec Chinh\nXin Viec Phu\nTu bo cong viec 1\nTu bo cong viec 2", "Chon", "Huy");
				return 1;
			}
			else
			{
...
if(newkeys & KEY_YES)
	{
	    ...
	    {
            ShowPlayerDialog(playerid, BANCA, DIALOG_STYLE_LIST, "{FF0000}NPC Thu Mua Ca", "Thu Mua Ca Chep Gia (200) SAD\n Thu Mua Rua Gia (1,000) SAD\n Thu Mua Ca Map Gia (1,500) SAD\n Thu Mua Ca Heo Gia (2,000) SAD", "Chon", "Thoat");
		    return 1;
        }
	}
...
if(newkeys & KEY_YES)
	{
        if(IsPlayerInRangeOfActor(playerid, npcbancals))
	    {
            ShowPlayerDialog(playerid, BANCA, DIALOG_STYLE_LIST, "{FF0000}NPC Thu Mua Ca", "Thu Mua Ca Chep Gia (200) SAD\n Thu Mua Rua Gia (1,000) SAD\n Thu Mua Ca Map Gia (1,500) SAD\n Thu Mua Ca Heo Gia (2,000) SAD", "Chon", "Thoat");
		    return 1;
        }
...
if(newkeys & KEY_YES)
{
	if(IsPlayerInRangeOfActor(playerid, Actor[7]))
		{
  			ShowPlayerDialog(playerid, BANVANG, DIALOG_STYLE_LIST, "TIEMVANG","Ban vang 30 Gram\nBan vang 50 Gram\nBan vang 60 Gram\nBan vang 80 Gram\nBan vang 100 Gram\nGia vang hien tai", "Chon", "Thoat");
              return 1;
		}
}
if(newkeys & KEY_YES)
    {
	    if(IsPlayerInRangeOfActor(playerid, NPCTetNguyenDan))
	    {
	         ShowPlayerDialog(playerid, DIALOG_TETNGUYENDAN, DIALOG_STYLE_LIST, "NPC Tet Nguyen Dan", "Xem Nguyen Lieu\nXem Banh Chung", "Chon", "Thoat");
	         return 1;
        }
    }
...
if(newkeys & KEY_YES)
    {
	    if(IsPlayerInRangeOfActor(playerid, NPCBanBanhChung))
	    {
...
{
	new Float:PosXACtor, Float:PosYACtor, Float:PosZACtor;
        GetActorPos(DaovangActor, PosXACtor, PosYACtor, PosZACtor);
        if(IsPlayerInRangeOfPoint(playerid, 2.0, PosXACtor, PosYACtor, PosZACtor))
        {
            if(PlayerInfo[playerid][pJob] != 35 && PlayerInfo[playerid][pJob2] != 35)
            {
       			ShowPlayerDialog(playerid, Job_DaoVang, DIALOG_STYLE_LIST, "Cong Viec Dao Vang", "Xin Viec Chinh\nXin Viec Phu\nTu bo cong viec 1\nTu bo cong viec 2", "Chon", "Huy");
				return 1;
			}
			else
			{
    ...
    {
        new Float:PosXACtor, Float:PosYACtor, Float:PosZACtor;
        GetActorPos(NpcNguoiThuMua, PosXACtor, PosYACtor, PosZACtor);
			if(IsPlayerInRangeOfPoint(playerid, 2.0, PosXACtor, PosYACtor, PosZACtor))
			{
		 		ShowPlayerDialog(playerid, DIALOG_NGUOITHUMUA, DIALOG_STYLE_LIST, "Nguoi Thu Mua", "Thit Ca\nWood\nStone\nGold", "Co", "Khong");
				return 1;
   			}
	}
 ...
 new Float:PosXACtor, Float:PosYACtor, Float:PosZACtor;
		//Actor Drugs Dealer
		GetActorPos(ActorDrugsDealer, PosXACtor, PosYACtor, PosZACtor);
        if(IsPlayerInRangeOfPoint(playerid, 5.0, PosXACtor, PosYACtor, PosZACtor))
        {
       		ShowPlayerDialog(playerid, ACTOR_DRUGSDEALER, DIALOG_STYLE_LIST, "NPC Drugs Dealer", "Ban can sa\nBan ma tuy", "Lua chon", "Huy bo");
			return 0;
        }		

		//Actor Arms Dealer
		GetActorPos(ActorArmsDealer, PosXACtor, PosYACtor, PosZACtor);
        if(IsPlayerInRangeOfPoint(playerid, 5.0, PosXACtor, PosYACtor, PosZACtor))
        {
       		ShowPlayerDialog(playerid, ACTOR_ARMSDEALER, DIALOG_STYLE_LIST, "NPC Arms Dealer", "Mua vu khi", "Lua chon", "Huy bo");
			return 0;
        }		
		
		// ActorPot
		GetActorPos(ActorPot, PosXACtor, PosYACtor, PosZACtor);
        if(IsPlayerInRangeOfPoint(playerid, 3.0, PosXACtor, PosYACtor, PosZACtor))
        {
       		ShowPlayerDialog(playerid, ACTOR_POT, DIALOG_STYLE_LIST, "NPC Buon can sa", "Huong dan cho nguoi moi\nVi tri san xuat", "Lua chon", "Huy bo");
			return 0;
        }		
		// ActorCrack
		GetActorPos(ActorCrack, PosXACtor, PosYACtor, PosZACtor);
        if(IsPlayerInRangeOfPoint(playerid, 3.0, PosXACtor, PosYACtor, PosZACtor))
        {
       		ShowPlayerDialog(playerid, ACTOR_CRACK, DIALOG_STYLE_LIST, "NPC Buon ma tuy", "Huong dan cho nguoi moi\nVi tri san xuat", "Lua chon", "Huy bo");
			return 0;
        }		
...
public OnPlayerCommandPerformed(playerid, cmdtext[], success)
{
    ...
    if(!success)
    {
        TextDrawShowForPlayer(playerid, Project_TD3[0]);
        TextDrawShowForPlayer(playerid, Project_TD3[1]);
        TextDrawShowForPlayer(playerid, Project_TD3[2]);
        
        SetTimerEx("HideSaiLenh", 2000, false, "i", playerid);
    }
    if(!success)
    {
        TextDrawShowForPlayer(playerid, Project_TD3[0]);
        TextDrawShowForPlayer(playerid, Project_TD3[1]);
        TextDrawShowForPlayer(playerid, Project_TD3[2]);
        SetTimerEx("HienSaiLenh", 2000, false, "i", playerid);
    }
    return 1;
}
...
public OnPlayerText(playerid, text[])
{
	...
	if(GetPVarInt(playerid, "DenNoiPhucVu") == 1)
	{
	    if(strcmp("Quy khach muon dung gi", text, true) == 0)
	    {
			...
			SetPlayerCheckpoint(playerid, 1817.0023, -1562.8186, 13.5487, 1);
		}
	}
	...
	{
		case 0, 1: accent = "{33CCFF}Binh Thuong{FFFFFF} ";
...
{
	//job giao ca
	if(checkpointid == CPGiaoCa1)
	
	{
		SendClientMessage(playerid,COLOR_WHITE,"{FFFFFF}[{6699FF}Giao Ca{FFFFFF}] Su Dung /layca De Bat Dau Cong Viec");
	    return 1;
	}

	if(checkpointid == CPGiaoCa2)
	{
	   	new PName[MAX_PLAYER_NAME];
		GetPlayerName(playerid,PName,sizeof(PName));
		if(GiaoCa[playerid] == 0)
		{
			SendClientMessage(playerid,COLOR_WHITE,"{FFFFFF}[{6699FF}Giao Ca{FFFFFF}] Hay Giao Con Ca Nay Toi Duoi Duong Ham Phia Truoc.");
		    return 1;
		}
        RemovePlayerAttachedObject(playerid, 1);
		ApplyAnimation(playerid,"CARRY","putdwn",1,0,0,0,0,0);
		SetPlayerSpecialAction(playerid,SPECIAL_ACTION_NONE);
	    SendClientMessage(playerid,COLOR_WHITE,"{FFFFFF}[{6699FF}Giao Ca{FFFFFF}] Ban Da Hoan Thanh Cong Viec, Nhan Duoc 3 KG Ca");
	    GiaoCa[playerid] =0;
		PlayerInfo[playerid][pThitCa] += 3;
		return 1;
	}
}
