...
public OnPlayerLoad(playerid)
{
    ...
    new string[1280];
    new var0;
	format(string, sizeof(string), "[Mobile]",playerid);
    SetPlayerChatBubble(playerid, string,COLOR_GREY,50.0,10000);
    SendClientMessage(playerid, COLOR_YELLOW, "[!] Ban da nhan duoc Tag Mobile" );
	}
	
	if(GetPVarInt(playerid, "NotAndroid") != 0)
	{
    new string[1280];
	format(string, sizeof(string), "[PC]",playerid);
    SetPlayerChatBubble(playerid, string,COLOR_LIGHTRED,50.0,10000);
    SendClientMessage(playerid, COLOR_YELLOW, "[!] Ban dan nhan duoc Tag PC" );
	}
	...
	if (PlayerInfo[playerid][pAdmin] > 0)
	{
		...
		else
		{
			...
			format(string, sizeof(string), "{ff0000} [ - ]May Chu : {00ff00}[ %s ] Da Dang Nhap Voi {ff0000} [ %s ]{FFFFFF}.", GetStaffRank(playerid));
			SendClientMessageEx(playerid, COLOR_WHITE, string);
			GetPlayerNameEx(playerid), GetStaffRank(playerid);
		}
	...
	format(string, sizeof(string), "~w~Chao Mung Tro Lai", GetPlayerNameEx(playerid));
    ...
    format(string, sizeof(string), "{00ff00}[ SB-RP.CF ] {ffffff}Chao Mung {ff0000}[ %s ] {ffffff}Den Voi May Chu {FA0000}[ SB-RP.CF ]{FFFFFF}", GetPlayerNameEx(playerid));
	SendClientMessageEx(playerid, COLOR_WHITE, string);
	SendClientMessageEx( playerid, COLOR_WHITE, "{00ff00}[ Goi Y ] {ffffff}Bam Phim {33ffcc}[ ALT ] {ffffff}De Mo Menu Nhanh Tai {FA0000}[ SB-RP.CF ]{FFFFFF}");
 	SendClientMessageEx( playerid, COLOR_WHITE, "{00ff00}[ Cai Dat ] {ffffff}Bam Phim {33ffcc}[ H ] {ffffff}De Mo Menu Cai Dat Tai Khoan Tai {FA0000}[ SB-RP.CF ]{FFFFFF}" );
    ...
    GameTextForPlayer(playerid, "~w~ Chao Mung Tro Lai", 5000, 1);
	SendClientMessageEx(playerid, COLOR_WHITE, string);
	...
	for(new xxx = 0; xxx < 6; xxx++) TextDrawShowForPlayer(playerid,LogoTD[xxx]);
	for(new xxx = 0; xxx < 2; xxx++) PlayerTextDrawShow(playerid,LogoPTD[xxx]);
	new young[64];
	format(young,sizeof(young),"ID: %d",playerid);
 	PlayerTextDrawSetString(playerid,LogoPTD[0],young);
 	format(young,sizeof(young),"Level: %d",PlayerInfo[playerid][pLevel]);
 	PlayerTextDrawSetString(playerid,LogoPTD[1],young);
 ...
 /*	new name[MAX_PLAYER_NAME];
    ...
    format(string128,sizeof(string128),"Ten Nguoi Choi[ %s ] Da Ket Noi Den Sever NG-RP", name);
    ABroadCast(COLOR_YELLOW, string128, 1);
    new ipp[MAX_PLAYER_NAME];
    GetPlayerIp(playerid,ipp,24);
    format(string128,sizeof(string128),"Ip Cua Nguoi Choi[ %s ] ",ipp);
    ABroadCast(COLOR_YELLOW, string128, 1);*/
    
	format(string128, sizeof(string128), "{ff0000}[ Kiem Soat ] {ffffff}Tai Khoan {00ff00}[ %s ] Dang Dang Nhap Voi {ff0000}IP Address : [ %s ]", GetPlayerNameEx(playerid), GetPlayerIpEx(playerid));
	ABroadCast(COLOR_YELLOW, string128, 1);
