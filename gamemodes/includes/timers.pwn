...
task thoigiann[1000]() {
    ...
    TextDrawSetString(LogoTD[5], string);
    format(string, sizeof(string), "%d/%d/%d", day, month,year);
    TextDrawSetString(LogoTD[4], string);
	return true;
}
...
task ProductionUpdate[300000]()
{
	...
	foreach(new i: Player)
	{
		...
		if(PlayerInfo[i][pConnectHours] < 2) {
			SendClientMessageEx(i, COLOR_LIGHTRED, "BAN SE KHONG DUOC SU DUNG VU KHI SAU KHI GIA NHAP MAY CHU SB-RP.CF 2H .");
		}
